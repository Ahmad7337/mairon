...
> **Requirements:** Python 3.8+, GCC (for C++ module), Kali-Linux

> **Packages:** `git` `python3` `pipx` `make` `g++` `libssl-dev` `pkg-config`
...
```bash
apt update && apt install -y git python3 pipx make g++ libssl-dev pkg-config
```
